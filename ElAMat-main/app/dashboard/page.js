...
export default function Dashboard() {
  ...
  useEffect(() => {
    const verifyAndFetch = async () => {
      ...
      // 1. استخراج الكود من التخزين المحلي
      const savedCode = typeof window !== 'undefined' ? localStorage.getItem("adminCode") : null;

      try {
        // 2. التحقق من الهوية عبر الـ API السري (لإخفاء الأرقام من الكود)
        const authRes = await fetch('/api/verify-admin', {
          method: 'POST',
        ...
        // لو السيرفر أكد إن الشخص ده أدمن، نبدأ نجيب الداتا
        if (authData.authorized) {
          ...
          // لو مش أدمن، نخليه false عشان الحماية
          setIsAdmin(false);
        }
      } catch (error) {
        console.error("Dashboard Security/Fetch Error:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  // حالة التحميل (الـ Spinner الخاص بك)
  if (loading) return (
  ...
  // إذا لم يكن أدمن بعد التحميل، لا تعرض شيئاً (أو رسالة خطأ)
  if (!isAdmin) return (
    <div className="min-h-screen flex items-center justify-center text-white font-bold">
      عذراً، لا تمتلك صلاحية الوصول لهذه البيانات.
    </div>
  );

  // العرض النهائي للـ UI الفخم
  return (
