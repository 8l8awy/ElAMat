...
function AdminContent() {
  ...
  const [showFake404, setShowFake404] = useState(false); // خليناها false عشان متبدأش تضرب في الوش
  ...
  // ✅ المحرك الأساسي: بيشتغل أول ما الصفحة تفتح ويقرأ الكود
  useEffect(() => {
  ...
  if (showFake404) return (
    <div style={{ backgroundColor: '#000', color: '#fff', height: '100vh', width: '100vw', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'fixed', top: 0, left: 0, zIndex: 99999, fontFamily: 'sans-serif' }}>
  ...
  if (!isAuthenticated) return (
    <div className="min-h-screen flex items-center justify-center p-6 text-white bg-black" dir="rtl">
      <div className="bg-[#111] p-10 rounded-[2.5rem] border border-white/10 w-full max-w-md text-center shadow-2xl">
        ...
        <button onClick={handleGoogleLogin} className="w-full flex items-center justify-center gap-3 bg-white text-black p-4 rounded-2xl font-black hover:bg-gray-200 mb-4"><FaGoogle /> دخول المشرفين بجوجل</button>
  ...
  return (
    <div className="min-h-screen w-full text-white p-4 md:p-8 font-sans bg-black" dir="rtl">
      <div className="max-w-7xl mx-auto">
        ...
        {/* باقي محتوى الصفحة (الرفع والأرشيف) شغال تمام كدة */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-20">
          <div className="lg:col-span-1">
            <div className="bg-[#111] p-6 rounded-[2rem] border border-white/5 sticky top-4 shadow-xl text-right">
              ...
              <form onSubmit={handleUpload} className="space-y-4">
                ...
                <input type="text" className="w-full bg-black border border-white/10 p-4 rounded-2xl outline-none" value={title} onChange={(e)=>setTitle(e.target.value)} placeholder="عنوان المخلص" required />
                <textarea className="w-full bg-black border border-white/10 p-4 rounded-2xl outline-none text-sm h-20 resize-none" value={desc} onChange={(e)=>setDesc(e.target.value)} placeholder="وصف المخلص..."></textarea>
                ...
                <button type="submit" disabled={uploading} className="w-full bg-purple-600 p-4 rounded-2xl font-black">{uploading ? "جاري النشر..." : "نشر الآن"}</button>
              </form>
            </div>
          </div>
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-[#111] p-6 rounded-[2rem] border border-white/5 shadow-2xl relative overflow-hidden text-right">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <h2 className="text-xl font-bold flex items-center gap-3 text-blue-500"><FaLayerGroup/> الأرشيف العام ({materialsList.length})</h2>
                <div className="relative w-full md:w-64">
                    <FaSearch className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 text-xs" />
                    <input type="text" placeholder="بحث سريع..." value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)} className="w-full bg-black/50 border border-white/5 p-2 pr-10 rounded-xl outline-none text-xs" />
                </div>
              </div>
              <div className="space-y-4 max-h-[800px] overflow-y-auto pr-2 custom-scrollbar">
                {filteredMaterials.map(item => (
                  <div key={item.id} className="bg-black/30 p-4 rounded-2xl flex justify-between items-center border border-white/5 hover:border-purple-500/30">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => window.open(item.files?.[0]?.url, '_blank')} className="p-3 rounded-xl bg-white/5 text-gray-500 hover:text-white"><FaLayerGroup size={14}/></button>
                      {adminRole === "admin" && ( <button onClick={() => { if(confirm("حذف؟")) deleteDoc(doc(db, "materials", item.id)) }} className="text-red-500/30 hover:text-red-500 p-3 rounded-xl"><FaTrash size={14}/></button> )}
                    </div>
                  </div>
                ))}
              </div>
