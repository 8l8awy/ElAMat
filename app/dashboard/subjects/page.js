...
export default function SubjectsPage() {
  // ✅ تعريف الـ States بقيم افتراضية أولية
  const [year, setYear] = useState(1);
  const [semester, setSemester] = useState(2);
  const [stats, setStats] = useState({});
  const [loading, setLoading] = useState(true);

  // ✅ 1. useEffect لتحميل القيم المحفوظة فور فتح الصفحة
  useEffect(() => {
    const savedYear = localStorage.getItem("lastSelectedYear");
    const savedSem = localStorage.getItem("lastSelectedSem");
    
    if (savedYear) setYear(Number(savedYear));
    if (savedSem) setSemester(Number(savedSem));
  }, []);

  // ✅ 2. دوال لتغيير السنة والترم وحفظهم في الذاكرة فوراً
  const handleYearChange = (y) => {
    setYear(y);
    localStorage.setItem("lastSelectedYear", y);
  };

  const handleSemChange = () => {
    const newSem = semester === 1 ? 2 : 1;
    setSemester(newSem);
    localStorage.setItem("lastSelectedSem", newSem);
  };
    ...
    <div className="min-h-screen w-full text-white px-2 md:px-10 py-6 font-sans overflow-x-hidden" dir="rtl">
      <div className="w-full max-w-7xl mx-auto mb-10 space-y-6">
        <div className="flex flex-wrap justify-between items-center gap-4 border-b border-white/5 pb-6">
           ...
           <button onClick={handleSemChange} 
        ...
        <div className="text-center pt-2">
          <h1 className="text-3xl md:text-5xl font-black italic tracking-tighter uppercase text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-500">المواد الدراسية</h1>
        </div>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-6 w-full max-w-7xl mx-auto pb-20 text-right">
        {subjects.map((sub) => (
          <Link href={`/dashboard/materials?subject=${encodeURIComponent(sub)}`} key={sub}>
            <div className="group bg-[#111] border border-white/5 rounded-[2rem] p-6 md:p-10 hover:bg-[#151515] transition-all duration-300 hover:-translate-y-1 cursor-pointer shadow-2xl relative overflow-hidden">
              ...
              <div className="flex flex-col items-center text-center space-y-4 md:space-y-6 relative z-10">
                ...
                <div className="flex items-center gap-2">
                   <div className="bg-black/40 px-4 py-2 rounded-xl text-[10px] font-bold text-gray-400 border border-white/5 flex items-center gap-2">
                     <span className="text-purple-500 italic">📚</span> {stats[sub]?.summary || 0} ملخص
                   </div>
                   <div className="bg-black/40 px-4 py-2 rounded-xl text-[10px] font-bold text-gray-400 border border-white/5 flex items-center gap-2">
                     <span className="text-orange-500 italic">📝</span> {stats[sub]?.assignment || 0} تكليف
                   </div>
                </div>
              </div>
            </div>
