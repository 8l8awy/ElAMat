...
function MaterialsContent() {
  ...
  // دالة توحيد نوع الملف (للتأكد من الفلترة الصحيحة)
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (subject) {
          ...
          // جلب كل المواد المقبولة إذا لم يتم تحديد مادة
  ...
  // ✅ الوظيفة الأساسية: فتح الملف وتحديث الإحصائيات
  const openFile = async (item) => {
    // 1. تحديد الرابط الصحيح
    ...
    if (url) {
        // 2. فتح الملف في نافذة جديدة فوراً
        window.open(url, '_blank');

        // 3. تحديث المشاهدات والتحميلات في الخلفية (دون انتظار)
        try {
        ...
        alert("عذراً، لا يوجد رابط صالح لهذا الملف.");
    }
  };
